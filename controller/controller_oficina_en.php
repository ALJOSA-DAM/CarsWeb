...
function listarOficinas(){
    ...
    include "view/index_oficina_en.php";
...
function eliminarOficinaController(){
    ...
    if (isset($_GET['id_oficina'])) {
            //if ($_SESSION['u_level'] == 0) {
                ...
                echo '<script>alert("Office Deleted");window.location.href="index2_en.php?action=listarOficinas&controller=controller_oficina_en"</script>';
...
function formularioAnadirOficina(){
   // iniciarSesion();
    require "view/new_oficina_en.php";
}

function anadirOficina() {
   // iniciarSesion();
    require_once "model/model_oficina_en.php";
  ...
  //  if (isset($_SESSION['user'])) {                                //Comprueba que la sesion este iniciada
        if (isset($_POST['submit']) && !empty($_POST['nombre'])) {   ////Comprueba que hay texto en el titulo y que se ha pulsado el boton de submit
            ...
            echo '<script>alert("Office Created");window.location.href="index2_en.php?action=listarOficinas&controller=controller_oficina_en"</script>';
            //header("Location:index2.php");
        } else {
            echo '<script>alert("You must name the office");window.location.href="index2_en.php?action=formularioAnadirOficina&controller=controller_oficina_en"</script>';
        }
