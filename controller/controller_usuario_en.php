...
function listarUsuarios(){
    ...
    include "view/index_usuario_en.php";
...
function eliminarUsuarioController(){
    ...
    if (isset($_GET['id_usuario'])) {
            //if ($_SESSION['u_level'] == 0) {
                ...
                echo '<script>alert("Usuario Eliminado");window.location.href="index2.php?action=listarUsuarios&controller=controller_usuario_en"</script>';
...
function formularioAnadirUsuario(){
   // iniciarSesion();
    require "view/new_usuario_en.php";
}
...
function anadirUsuario() {
  ...
  //  if (isset($_SESSION['user'])) {                                //Comprueba que la sesion este iniciada
        if (isset($_POST['submit']) && !empty($_POST['nombre'])) {   ////Comprueba que hay texto en el titulo y que se ha pulsado el boton de submit
            ...
            echo '<script>alert("User Created");window.location.href="index2_en.php?action=listarUsuarios&controller=controller_usuario_en"</script>';
            //header("Location:index2.php");
        } else {
            echo '<script>alert("You must enter a name");window.location.href="index2_en.php?action=formularioAnadirUsuario&controller=controller_usuario_en"</script>';
        }
